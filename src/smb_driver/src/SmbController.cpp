...
SmbController::SmbController(std::string port, std::shared_ptr<rclcpp::Node> node, size_t vecSize, bool sendCommands, double lin_vel_scale, double ang_vel_scale) :
        Node("smb_controller"),
...
//  this->startAcquisition();
  ...
  wheelSpeedPub_ = this->create_publisher<std_msgs::msg::Float64MultiArray>("wheelSpeeds", 1);
  rcTwistPub_ = this->create_publisher<geometry_msgs::msg::Twist>("rc_twist", 1);
...
bool SmbController::readWheelSpeeds() {
    ...
    if (res) {
      ...
      wheelSpeedMsg_.data[0] = rclcpp::Clock{}.now().seconds();
